...
void PlaneImpl::insert_point(const pcl::PointXYZITC &point)
{
  ...
  // point_num_++;
  points.points.push_back(point); // tmp
}
...
void PlaneImpl::update()
{
  ...
  scalar_t valid_points_num = 0;
  for (int i = 0; i < points.points.size(); i++)
  {
    auto &pw = points.points[i];
    vector3_t pw_eigen = pw.xyz().cast<scalar_t>();
    covariance += pw_eigen * pw_eigen.transpose();
    center += pw_eigen;
    valid_points_num +=1;
  }
  if (valid_points_num < CONSTRUCT_THRESHOLD) 
  {
    is_valid_ = false;
    return;
  }
  center /= valid_points_num;
  covariance = covariance / valid_points_num - center * center.transpose();
  scalar_t covariance_norm = covariance.norm();
  if (covariance_norm < scalar_t(1e-6)) 
  {
    is_valid_ = false;
    return;
  }
  // PCA
  Eigen::EigenSolver<matrix3_t> es(covariance);
  matrix3c_t eigen_vecs = es.eigenvectors();
  vector3c_t eigen_vals = es.eigenvalues();
  vector3_t eigen_vals_real = eigen_vals.real();
  matrix3_t::Index eigen_vals_min, eigen_vals_max;
  ...
  vector3_t eigen_vec_mid = eigen_vecs.real().col(3 - eigen_vals_min - eigen_vals_max);
  vector3_t eigen_vec_max = eigen_vecs.real().col(eigen_vals_max);
  // Plane uncertainty calculation
  matrix3_t J_Q = 1.0 / valid_points_num * matrix3_t::Identity();
  matrix6_t plane_uncertainty = matrix6_t::Zero();
  for (int i = 0; i < points.points.size(); i++)
  {
    auto &pw = points.points[i];
    ...
    for (int m = 0; m < 3; m++)
    {
      Eigen::Matrix<scalar_t, 1, 3> F_m = Eigen::Matrix<scalar_t, 1, 3>::Zero();
      if (m != (int)eigen_vals_min)
      {
        vector3_t pw_eigen = pw.xyz().cast<scalar_t>();
        F_m = (
        (pw_eigen - center).transpose() / (valid_points_num * (eigen_vals_real[eigen_vals_min] - eigen_vals_real[m])) *
        (eigen_vecs.real().col(m) * eigen_vecs.real().col(eigen_vals_min).transpose() + eigen_vecs.real().col(eigen_vals_min) * eigen_vecs.real().col(m).transpose())
        );
      }
      F.row(m) = F_m;
    }
    J.block<3, 3>(0, 0) = eigen_vecs.real() * F;
    J.block<3, 3>(3, 0) = J_Q;
    plane_uncertainty += J * pw.covariance().cast<scalar_t>() * J.transpose();
  }
  center_ = center;
  covariance_ = covariance;
  uncertainty_ = plane_uncertainty;
  normal_ = eigen_vec_min.normalized();
  main_axis_ = eigen_vec_max.normalized();
  secondary_axis_ = eigen_vec_mid.normalized();
  radius_ = sqrt(eigen_vals_real(eigen_vals_max));
