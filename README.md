...
```
  ...
  ros2 launch gs-lio launch_with_bag.launch.py 
```
3. open foxglove and check visualize result
...
6. Split project into two part: seperate into lio and triangle splatting part, now it is 2 node, so I think can be easily seperate.

7. Shrink docker image size: now is 41GB, not sure why.
...
##### 2026/04/19更新：完成初步的三角化，運行範例後可以透過foxglove看到/triangle_splatting/combiined的結果。
...
6. 分割專案：分成lio和triangle splatting的部分，目前功能上已經是兩個節點了應該不難切。

7. 縮小docker映像大小：現在是41GB，感覺除了弄成runner builder沒別的方法，但目前還要開發應該是不太能縮小。
